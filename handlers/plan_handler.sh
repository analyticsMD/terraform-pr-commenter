...
plan_fail () {
  ...
  post_diff_comments "plan" "Terraform \`plan\` Failed for Workspace: \`$WORKSPACE\`, Component: \`$COMPONENT\`" "$clean_input"
}
...
post_plan_comments () {
  ...
  post_diff_comments "plan" "Terraform \`plan\` Succeeded for Workspace: \`$WORKSPACE\`, Component: \`$COMPONENT\`.\n\`$OVERVIEW\`" "$clean_input"
}
